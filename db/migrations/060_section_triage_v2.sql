...
SET dify_eligible = true,
    triage_version = 'legacy-preserved',
    triage_category = 'general_market_news',
    triage_score = section_priority,
    triage_reasons = '["existing retry state preserved"]'::jsonb,
    triaged_at = now()
WHERE fact_extraction_status IN ('failed_retryable', 'failed_terminal')
   OR (fact_extraction_status = 'pending' AND fact_extraction_attempts > 0);
