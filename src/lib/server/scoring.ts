/**
 * Dynamic authenticity scoring engine — Phase 1.
 *
 ...
 *
 * Sanction override:
 *   listed  → hard floor: total ≤ 10, all dimensions forced to minimum
 *   unknown → all computed sub-scores × 0.7
...
function scoreVessel(inputs: ScoringInputs) {
  ...
  if (aisData?.mmsi)               E += 8    // MMSI resolved → AIS-registered
...
function scoreCompany(inputs: ScoringInputs) {
  const { country, registrationNumber, intelligence } = inputs
  let E = 0, A = 0, D = 0, C = 0
...
function scoreTerminal(inputs: ScoringInputs) {
  const { country, intelligence } = inputs
  let E = 0, A = 0, D = 0, C = 0
  ...
  const sanctionHits = intelligence?.sanctions_hits?.length ?? 0
  C += sanctionHits === 0 ? 6 : 0
  C += (intelligence?.risk_signals?.length ?? 0) === 0 ? 4 : 2
