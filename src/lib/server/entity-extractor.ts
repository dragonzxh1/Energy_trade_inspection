...
export async function extractTradeParams(text: string): Promise<ExtractedTradeParams | null> {
  ...
  try {
    // enable_thinking: false — disables chain-of-thought reasoning.
    // Without it, qwen3.5-plus uses ~4000 reasoning tokens per call (80s latency).
    // With it disabled, latency drops to ~8s and token usage drops 66%.
