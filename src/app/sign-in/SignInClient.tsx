...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  OAuthCreateAccount:   'Account creation failed. Please try again.',
  EmailCreateAccount:   'Account creation failed. Please try again.',
  Callback:             'Sign-in callback failed. Please try again.',
  OAuthAccountNotLinked:'This email is already registered with another sign-in method.',
  CredentialsSignin:    'Invalid email or password.',
  invalid_token:        'Verification link is invalid. Please register again.',
  token_expired:        'Verification link has expired. Please register again.',
  invalid_link:         'Verification link is malformed.',
}
...
export default function SignInClient({ callbackUrl, verified, errorCode }: Props) {
  ...
  const errorMsg = formError ?? (errorCode ? (ERROR_MESSAGES[errorCode] ?? 'Sign-in failed. Please try again.') : null)
    ...
    >
      <div style={{ width: '100%', maxWidth: '380px' }}>
        <div style={{ textAlign: 'center', marginBottom: 'var(--space-8)' }}>
          <Link href="/" style={{ color: 'var(--text-primary)', textDecoration: 'none', fontSize: '20px', fontWeight: 700, letterSpacing: '-0.02em' }}>
            ETI
          </Link>
        ...
        <div style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: '12px', padding: 'var(--space-8)' }}>
          <h1 style={{ fontSize: '20px', lineHeight: '28px', fontWeight: 600, marginBottom: 'var(--space-2)', textAlign: 'center' }}>
            Sign in
          </h1>
          ...
          {verified && (
            <div style={{ backgroundColor: 'var(--success-bg, #f0fdf4)', border: '1px solid var(--success-border, #bbf7d0)', borderRadius: '6px', padding: '10px 12px', marginBottom: 'var(--space-4)', fontSize: '13px', color: 'var(--success-text, #166534)' }}>
          ...
          <div style={{ display: 'flex', border: '1px solid var(--border-subtle)', borderRadius: '8px', overflow: 'hidden', marginBottom: 'var(--space-5)' }}>
          ...
          {tab === 'google' && (
            <button
              ...
              disabled={loading}
              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'var(--space-3)', padding: 'var(--space-3) var(--space-4)', backgroundColor: '#fff', color: '#1f2937', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 500, fontFamily: 'inherit', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}
            >
          ...
          {tab === 'email' && (
            <form onSubmit={handleCredentialsSignIn}>
              ...
              <div style={{ textAlign: 'right', marginBottom: 'var(--space-4)' }}>
                <Link href="/forgot-password" style={{ fontSize: '12px', color: 'var(--accent-primary)' }}>
                  Forgot password?
                </Link>
              </div>
          ...
          <p style={{ marginTop: 'var(--space-5)', color: 'var(--text-muted)', fontSize: '11px', textAlign: 'center', lineHeight: '17px' }}>
            By signing in, you agree to our Terms of Service and Privacy Policy. Free plan: 5 queries/month.
          </p>
        </div>

        <p style={{ textAlign: 'center', marginTop: 'var(--space-4)', color: 'var(--text-muted)', fontSize: '12px' }}>
          Don&apos;t have an account?{' '}
          <Link href="/sign-up" style={{ color: 'var(--accent-primary)' }}>Sign up</Link>
          {' · '}
